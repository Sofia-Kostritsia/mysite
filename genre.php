...
try {
    ...
    $genre = isset($_GET['genre']) ? intval($_GET['genre']) : null;

    if ($genre) {
        // Підготовка та виконання SQL-запиту для вибраної книги
        $stmt = $conn->prepare("SELECT ID_Book, NameOfBook, Author, Genre, TypeOf, CountOfPages, YearOf, Vydavnytstvo, ISBN FROM Books WHERE Genre = :genre");
